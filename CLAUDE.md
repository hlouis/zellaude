...
**Activity state machine.** Each Claude session (keyed by `pane_id`) has an `Activity`. Hook events map to activities in `event_handler.rs`: `PreToolUse → Tool(name)`, `PostToolUse/UserPromptSubmit → Thinking`, `PermissionRequest → Waiting`, `Stop → Done`, `SubagentStop → AgentDone`, `SessionStart → Init`, `SessionEnd → remove`. `Notification` only refreshes the timestamp without changing activity. `Done`/`AgentDone` decay to `Idle` after `DONE_TIMEOUT` (30s) via the timer. When a tab has multiple sessions, render picks the highest `activity_priority` one.
