...
**Focus reset.** `State.focused_pane` tracks the focused terminal pane (from `PaneInfo.is_focused` in the active tab, recomputed by `refresh_focus` on every `PaneUpdate`/`TabUpdate`). Focusing a pane that is `Prompting` resets it to `Idle` and clears its flash — once you're looking at a pane, its `▶` has done its job. `event_handler` also suppresses `▶`/flash up front when the Stop/Notification arrives for the already-focused pane. This applies only to `Prompting`; `Waiting` (permission) is untouched, since focusing isn't answering. **The reset is broadcast** (`zellaude:ack`, see Multi-instance sync): the bar is rendered by whichever instance owns the *active* tab, and a background instance lacking accurate focus would otherwise keep showing the stale `▶` the moment you switch tabs. So both the `refresh_focus` reset and the up-front suppression broadcast an ack; every instance clears that pane's `▶`/flash on receipt.

**Acknowledgement memory.** A pane that has been looked at goes into `State.acked_panes` (recorded in `refresh_focus`, the up-front suppression in `event_handler`, and `apply_ack` so every instance agrees). Claude Code fires `Notification` *repeatedly* while a session sits waiting, so without this a focused-then-backgrounded pane would re-flash on every nudge — the "tab flashes forever even though I looked at all its panes" bug. While a pane is acked, repeat `Notification`s are gated (no flash, `▶` stays suppressed). The ack is forgotten the moment Claude resumes work (`PreToolUse`/`PostToolUse`/`UserPromptSubmit`/`SessionStart` → `acked_panes.remove`), so the *next* genuine wait flashes normally. `PermissionRequest` is never gated — it is blocking and always flags.

**Attention flashing.** `should_flash` (event_handler) drives the tab flash: `PermissionRequest` (loud — the hook script also fires a macOS desktop notification) and `Notification` (quiet nudge — flash only, no desktop notification; this is the "you've left Claude waiting" case). A plain `Stop` shows `▶` without flashing. Flash respects the `flash` setting (Once/Persist/Off) and clears when you focus the tab or the session moves off a needs-you state; repeat `Notification`s for an already-acked pane are suppressed (see Acknowledgement memory). **Desktop notifications fire for `PermissionRequest` (loud, plus a terminal bell) and `Stop` ("done — your turn")** — and only from the hook script (one per pane), never the plugin, to avoid N-instance duplicates.

**Focus publishing (`Notify: unfocused`).** The `unfocused` notification mode must answer "are you looking at this pane's Zellij tab?", but the hook is a plain subprocess that knows only its own `$ZELLIJ_PANE_ID` — the pane → tab mapping lives in the plugin. So `State::publish_focus` writes the on-screen tab's terminal pane ids to `/tmp/zellaude-focus-<session>.json` on every `TabUpdate`/`PaneUpdate` (debounced via `published_focus`; also re-run in `PermissionRequestResult`, since earlier `run_command`s were dropped). Every instance writes the same content — no leader election — and the write is temp-file + `mv`, so racing writers only ever swap in identical bytes and a reader never sees a partial file. The hook then rejects on three independent layers, cheapest first: pane not in the on-screen tab (this file) → terminal window title doesn't start with the session name (which native terminal tab is on top; Ghostty's AppleScript dictionary on macOS, `xdotool getactivewindow getwindowname` on X11) → terminal app not frontmost. A layer whose evidence is missing is **skipped, not guessed**, so a missing signal costs an extra notification rather than a swallowed one. Deliberately *not* covered: which pane inside the on-screen tab has the cursor — the bar's `▶`/flash is already in view there.
